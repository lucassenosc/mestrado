"""
Carlini-Wagner L2 attack adapted from CleverHans 4.0.0.

Local compatibility changes:
1. Use a per-sample binary-search constant.
2. Support the current Keras optimizer.variables API.
3. Expand masks dynamically to support tabular inputs.

The attack objective and optimization procedure are otherwise unchanged.
"""
    ...
    def _attack(self, x):
        ...
        for outer_step in range(self.binary_search_steps):
            ...
            for iteration in range(self.max_iterations):
                ...
                # mask is of shape [batch_size]; best_attack is [batch_size, image_size]
                # need to expand
                extra_dims = len(best_attack.shape) - 1
                mask = tf.reshape(mask, [-1] + [1] * extra_dims)
                mask = tf.tile(mask, [1, *best_attack.shape[1:]])
